...
try {
  ...
  if (JSON.stringify(reference.collections.enemies.items?.families?.map((family) => family.id)) !== JSON.stringify(["charger", "ranged", "flyer", "bomber", "armored", "priest", "mender", "herald", "anchor", "wraith", "chimera", "rootbinder"])) throw new Error("game reference check produced an invalid public enemy-family catalog");
  ...
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.id)) !== JSON.stringify(["warden", "colossus", "aldric", "rootbound", "echo", "source"])) throw new Error("game reference check produced an invalid public boss catalog order");
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.name)) !== JSON.stringify(["The Warden", "Iron Colossus", "Berserker King", "The Rootbound", "The Echo", "The Source"])) throw new Error("game reference check produced invalid public boss names");
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.stageId)) !== JSON.stringify(["grounds", "undercroft", "crimson-fields", "verdant-sanctum", "voidspire", "tear"])) throw new Error("game reference check produced an invalid public boss/stage mapping");
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.phaseMarks)) !== JSON.stringify([[0.65, 0.30], [0.60, 0.25], [0.65, 0.20], [0.65, 0.28], [0.60, 0.25], [0.58, 0.28]])) throw new Error("game reference check produced invalid boss phase marks");
  if (JSON.stringify(reference.collections.stages?.items?.map((stage) => stage.id)) !== JSON.stringify(["grounds", "undercroft", "crimson-fields", "verdant-sanctum", "voidspire", "tear"])) throw new Error("game reference check produced an invalid public-stage catalog");
  if (reference.collections.stages?.status !== "complete" || reference.collections.stages.items?.length !== 6) throw new Error("game reference check produced an incomplete six-stage public catalog");
  const publicManifest = JSON.stringify(reference.collections);
  for (const leakedId of ["pale-traverse", "white-hart", "rimehound", "rime-runner", "prism-seer", "snowfall-kite", "hailcaster", "glacier-guard"]) {
    if (publicManifest.includes(leakedId)) throw new Error(`game reference check leaked Playground-only identity ${leakedId}`);
  }
