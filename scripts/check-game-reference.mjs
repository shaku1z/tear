...
try {
  ...
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.id)) !== JSON.stringify(["warden", "colossus", "aldric", "rootbound", "echo", "source"])) throw new Error("game reference check produced an invalid boss catalog order");
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.name)) !== JSON.stringify(["The Warden", "Iron Colossus", "Berserker King", "The Rootbound", "The Echo", "The Source"])) throw new Error("game reference check produced invalid authored boss names");
  if (JSON.stringify(reference.collections.bosses.items?.map((boss) => boss.stageId)) !== JSON.stringify(["grounds", "undercroft", "crimson-fields", "verdant-sanctum", "voidspire", "tear"])) throw new Error("game reference check produced an invalid boss/stage mapping");
  ...
  if (JSON.stringify(reference.collections.stages?.items?.map((stage) => stage.id)) !== JSON.stringify(["grounds", "undercroft", "crimson-fields", "verdant-sanctum", "voidspire", "tear"])) throw new Error("game reference check produced an invalid engineering-stage catalog");
  if (reference.collections.stages?.status !== "complete" || reference.collections.stages.items?.length !== 6) throw new Error("game reference check produced an incomplete six-stage intermediate catalog");
