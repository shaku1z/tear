...
The design system lives in `js/ui.js` as the `UI` object: a token layer (`UI.t`) plus a set of canvas components (`UI.*`). All menu, HUD, and overlay UI must be drawn through it — never hand-roll text, panels, or buttons at the call site. In-world game art (enemies, the player, particles, biome backdrops) is exempt; this system is only for chrome/UI.
...
1. **Add a method to `UI`** in `js/ui.js`, alongside the existing components, following the `(ctx, …)` signature shape.
