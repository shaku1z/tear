...
const Backdrop = {
  ...
  motes(ctx, c, t, px, style) {
    ...
    for (const p of c.parts) {
      const y = ((p.y + dir * t * p.sp) % (H + 80) + (H + 80)) % (H + 80) - 40;
...
const BIOME_ART = {
  ...
  "The Grounds": {
    sky(B, ctx, stage, c, t, gy) {
      ...
      ctx.fillStyle = g; ctx.fillRect(0, 0, B.W, gy);
      ...
      ctx.fillStyle = sg; ctx.fillRect(0, 0, B.W, B.H);
    ...
    far(B, ctx, stage, c, t, px, gy) {
      ...
      for (let x = 130; x < B.W; x += 175) { const cx = x + off; ctx.fillRect(cx, top, cw, ch); ctx.fillRect(cx - 6, top - 10, cw + 12, 12); ctx.fillRect(cx - 6, gy - 8, cw + 12, 8); }
      ctx.restore();
    },
  ...
  "The Undercroft": {
    sky(B, ctx, stage, c, t, gy) {
      ...
      ctx.fillStyle = fg; ctx.fillRect(0, 0, B.W, B.H);
    },
    far(B, ctx, stage, c, t, px, gy) {
      ctx.save(); const col = B._darken(stage.bg, 0.22), off = -px * 30;
      ctx.globalAlpha = 0.55; ctx.fillStyle = col;
      for (let x = 60; x < B.W; x += 260) { const cx = x + off; ctx.fillRect(cx, gy - 220, 70, 220); ctx.fillRect(cx - 30, gy - 150, 150, 18); }
  ...
  "The Voidspire": {
    sky(B, ctx, stage, c, t, gy) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let x = 0; x <= B.W; x += 60) ctx.lineTo(x, yb + Math.sin(x * 0.005 + t * 0.4 + i) * amp);
        ctx.lineTo(B.W, 0); ctx.lineTo(0, 0); ctx.closePath(); ctx.fill();
      }
      ctx.restore();
    },
    far(B, ctx, stage, c, t, px, gy) {
