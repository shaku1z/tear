...
(function () {
  ...
  // ---- world geometry ----
  const platforms = [
    { x: 0, y: CONFIG.world.groundY, w: W, h: H - CONFIG.world.groundY },
    { x: 180, y: 470, w: 240, h: 24 },
    { x: 860, y: 470, w: 240, h: 24 },
  ...
  function dealAoE(cx, cy, radius, dmg) {
    for (const e of enemies) {
      ...
      if (len(e.x - cx, e.y - cy) <= radius + e.radius) {
        ...
        if (e.dead) { run.score += CONFIG.run.scorePerKill * run.wave; FX.death(e.x, e.y, CONFIG.juice.deathShards); }
      }
    }
  }
  ...
  function endRun() {
    ...
    overInfo = { wave: run.wave, score: run.score, best: getBest(run.mode, run.diff), prevBest: best, isNew };
    state = "gameover";
  ...
  function stepPlaying(dt) {
    ...
    if (blade.thrown) {
      for (const e of enemies) {
        ...
        if (segCircle(blade.x, blade.y, blade.tipX, blade.tipY, e.x, e.y, e.radius + 4)) {
          blade.pierced.add(e);
          e.hit(blade.throwDmg, blade.vx, blade.vy);
          FX.burst(e.x, e.y, blade.vx, blade.vy, CONFIG.juice.sparkCount);
          addFloater(e.x, e.y - 26, Math.round(blade.throwDmg).toString(), true);
    ...
    for (const p of projectiles) {
      ...
      if (p.deflected) {
        ...
        if (player.takeDamage(CONFIG.proj.dmg, p.x)) p.dead = true;
      }
    }
    ...
    for (const e of enemies) {
      ...
      if (aabbOverlap(player.x, player.y, player.hw, player.hh, e.x, e.y, e.hw, e.hh)) {
        player.takeDamage(e.cfg.contactDmg, e.x);
      }
    }
  ...
  function onKill(e) {
    run.score += CONFIG.run.scorePerKill * run.wave;
  ...
  function drawHUD() {
    ...
    UI.title(ctx, "WAVE " + run.wave, W / 2, 40, 26);
    ctx.textAlign = "right";
  ...
  function renderHowto() {
    ...
    const lines = [
      ...
      "Blade: move the mouse — click to capture it",
      "       it carries momentum; damage = how fast the tip moves",
  ...
  function renderDraft() {
    ...
    draftChoices.forEach((up, i) => {
      ...
      ctx.fillStyle = "#000";
      UI.text(ctx, up.kind === "stat" ? "STAT" : "EFFECT", x + cw / 2, y0 + 40, 14, "center", 0.5);
  ...
  function renderGameover() {
    UI.dim(ctx, W, H, 0.85);
    UI.title(ctx, "DEFEATED", W / 2, 180, 56);
    UI.text(ctx, "reached wave " + overInfo.wave + "   ·   " + overInfo.score + " pts", W / 2, 240, 22, "center");
    if (overInfo.isNew) UI.title(ctx, "NEW BEST!", W / 2, 290, 26);
    else UI.text(ctx, "best: wave " + overInfo.best.wave + " · " + overInfo.best.score + " pts", W / 2, 290, 18, "center", 0.6);
    vmenu([
