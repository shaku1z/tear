...
describe("Pale mode and lifecycle coverage", () => {
  it("projects every canonical enemy identity into Playground and Enemy Test", () => {
    ...
    expect(seen.has("rimehound")).toBe(true);
  });

  it.each(["endless", "gauntlet"] as const)("uses the authored Pale pool when %s rotates into the biome", (mode) => {
  ...
  it("keeps Campaign and Boss Test on the same Pale and White Hart authorities", () => {
    ...
    const whiteHartIndex = BOSS_ROSTER.findIndex((boss) => boss.id === "white-hart");
    const bossTest = plan({ state: state({ mode: "bossonly", wave: whiteHartIndex, bossIdx: whiteHartIndex }), bossOnly: true });
    expect(bossTest.state).toMatchObject({ curBoss: "white-hart", stage: 4, currentStageIndex: 4 });
  });
