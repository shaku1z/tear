...
describe("normal player Watch controller", () => {
  it("is visibly unavailable when no validated local candidate can load", async () => {
    const controller = new LivePlayerWatchController(undefined, port(), async () => undefined);
  ...
  it("uses only a loaded canonical decision and restores native authority on pause, resume, and stop", async () => {
    ...
    const controller = new LivePlayerWatchController(undefined, target, async () => runtime as never);
    ...
    expect(target.authority.mock.calls.map(([value]) => value)).toEqual([true, false, true, false]);
