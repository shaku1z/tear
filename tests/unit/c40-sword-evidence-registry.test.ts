...
describe("C40 Sword Seam/Crosscut/catch seek evidence registry", () => {
  it("names the narrow non-certifying browser proof", () => {
    ...
    expect(catalog.find((entry) => entry.id === "sword-seam-crosscut-catch-seek")).toMatchObject({
  ...
  it("routes only Sword transport and V3 seek proof without fabricated graveyard coverage", () => {
    ...
    expect(routes.find((entry) => entry.id === "sword-seam-crosscut-replay")).toMatchObject({ scenarios: ["sword-seam-crosscut-catch-seek"], graveyardCases: [], journeyCheckpoint: "sword-seam-crosscut-catch-seek-engineering-proof", baseComparison: "not-run-by-this-scenario", interactionMatrices: ["browser"] });
  });
