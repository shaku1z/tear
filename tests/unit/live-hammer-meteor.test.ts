...
describe("live hammer meteor", () => {
  it("scales the impact, breaks nearby enemies, and redirects to a distant cluster", () => {
    ...
    const areaDamage = vi.fn(() => 0), ribbon = vi.fn(), presentAttack = vi.fn();
    const impact = bindLiveHammerMeteor({
      blade: () => ({ vx: 1000, vy: 500, throwDmg: 100, throwOrigin: { x: -400, y: 0 }, attackId: 7, throwId: 3 }),
      ...
      explode: vi.fn(), ribbon, shake: vi.fn(), zoom: vi.fn(), boom: vi.fn(), areaDamage, presentAttack,
    });
    impact(0, 0);
    ...
    expect(presentAttack).toHaveBeenCalledWith(expect.objectContaining({
      weaponId: "hammer", attackId: 7, throwId: 3, variant: "meteor", phase: "impact", x: 0, y: 0,
    }));
  });
});
