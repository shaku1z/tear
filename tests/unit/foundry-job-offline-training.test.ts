...
import { TearFoundryJobVault, TearFoundryOfflineTrainingExecutor, createTearFoundryJob, parseTearFoundryOfflineTrainingLaunch, transitionTearFoundryJob, type TearAcademyCandidateCustodyStore, type TearAcademyCorpusStore, type TearAcademyTrainingDatasetLoader } from "../../src/agents";
...
describe("C36 offline training launch", () => {
  it("refuses V1 launch bytes because their plan/configuration cannot be recovered", () => {
    expect(() => parseTearFoundryOfflineTrainingLaunch({ format: "tear-foundry-offline-training-launch", schemaVersion: 1, launchHash: "a".repeat(16) })).toThrow(/non-resumable/u);
  });
