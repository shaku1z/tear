...
describe("C36 app-owned local Foundry scheduler", () => {
  it("starts from durable discovery, runs one due schedule, serializes overlap, and schedules the next bounded wake", async () => {
    ...
    const scheduler = new LiveFoundryScheduler({ discover: vi.fn(() => Promise.resolve([due(), { ...due(), scheduleHash: "c".repeat(16) }])), execute, now: () => new Date("2026-08-08T00:01:00.000Z"), defer: (fn) => { callback = fn; return 1 as unknown as ReturnType<typeof setTimeout>; }, cancel: vi.fn(), leaseId: () => "lease" });
  ...
  it("rediscovers after restart, never executes disabled or blocked work, and exposes truthful local statuses", async () => {
    const execute = vi.fn();
    const ports = { discover: vi.fn(() => Promise.resolve([{ ...due(), state: "disabled" as const, disposition: "disabled" as const }, { ...due(), scheduleHash: "c".repeat(16), disposition: "blocked-revoked-custody" as const }])), execute, now: () => new Date("2026-08-08T00:01:00.000Z"), defer: () => 1 as unknown as ReturnType<typeof setTimeout>, cancel: vi.fn(), leaseId: () => "lease" };
  ...
  it("retains an execution failure as error without retrying a second schedule in that wake", async () => {
    ...
    const scheduler = new LiveFoundryScheduler({ discover: () => Promise.resolve([due(), { ...due(), scheduleHash: second }]), execute, now: () => new Date("2026-08-08T00:01:00.000Z"), defer: () => 1 as unknown as ReturnType<typeof setTimeout>, cancel: vi.fn(), leaseId: () => "lease" });
