...
import {
  environmentHash, environmentSnapshotToObservation,
...
describe("Pale Aurora environment definitions", () => {
  ...
  it("fails closed on malformed direction, carry, lifecycle, caps, and collection category", () => {
    const runtime = createEnvironmentRuntime({ stageId: "pale-traverse", worldId: "pale-malformed", validators: [PALE_TRACK_VALIDATION] });
    ...
    expect(() => runtime.addRoute({ ...route, maximumConcurrent: 4 })).toThrow(/remain three/u);
