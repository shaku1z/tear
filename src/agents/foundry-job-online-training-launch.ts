import type { GhostVaultStore, GhostVaultWrite } from "../ghost";
...
export interface TearFoundryOnlineTrainingExecutionContinuationV1 { readonly guards: (output: Readonly<{ job: TearFoundryJobV1; launch: TearFoundryOnlineTrainingLaunchV1; receipt: TearFoundryOnlineTrainingExecutionReceiptV1; checkpoint: TearOnlineRlCheckpointV1 }>) => readonly Readonly<{ store: GhostVaultStore; key: string; expected?: string }>[]; readonly writes: (output: Readonly<{ job: TearFoundryJobV1; launch: TearFoundryOnlineTrainingLaunchV1; receipt: TearFoundryOnlineTrainingExecutionReceiptV1; checkpoint: TearOnlineRlCheckpointV1 }>) => readonly GhostVaultWrite[]; }
...
export class TearFoundryOnlineTrainingExecutionExecutor {
  ...
  async execute(jobInput: TearFoundryJobV1, readinessInput: TearFoundryEvaluationReadinessReceiptV1, launchHash: string, executedAt: string, control: TearOnlineRlAdvanceControl = {}, continuation?: TearFoundryOnlineTrainingExecutionContinuationV1): Promise<Readonly<{ job: TearFoundryJobV1; launch: TearFoundryOnlineTrainingLaunchV1; receipt: TearFoundryOnlineTrainingExecutionReceiptV1 }>> {
    const job = parseTearFoundryJob(jobInput), readiness = parseTearFoundryEvaluationReadinessReceipt(readinessInput), backend = this.#jobs.backend();
    ...
    const output = Object.freeze({ job: next, launch: nextLaunch, receipt, checkpoint: advanced }), guards = continuation?.guards(output) ?? Object.freeze([]), writes = continuation?.writes(output) ?? Object.freeze([]);
    await this.#jobs.persistSuccessorIfMatches(job, next, guards, Object.freeze([{ store: "analysis", key: `online-rl-checkpoint:v1:${advanced.checkpointHash}`, value: JSON.stringify(advanced) }, { store: "analysis", key: `${KEY}${nextLaunch.launchHash}`, value: JSON.stringify(nextLaunch) }, { store: "analysis", key: `foundry-job-online-training-execution:v1:${receipt.receiptHash}`, value: JSON.stringify(receipt) }, { store: "indexes", key: `foundry-job-online-training-execution:${job.id}:${receipt.receiptHash}`, value: JSON.stringify(Object.freeze({ status: advanced.status, checkpointHash: advanced.checkpointHash })) }, ...writes]));
