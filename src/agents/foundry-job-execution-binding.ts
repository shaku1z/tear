...
const V3_KEY = "foundry-job-execution-binding:v3:";
...
export type TearFoundryExecutionSuccessorDeclarationV3 =
  | Readonly<{ kind: "exact-phase-payload"; payload: TearFoundryExecutionBindingPayloadV1; nextDeclaration: TearFoundryExecutionSuccessorDeclarationV3 }>
  | Readonly<{ kind: "emitted-v2-training"; nextDeclaration: Readonly<{ kind: "repeat-v2-training" }> }>
  | Readonly<{ kind: "repeat-v2-training" }>;
export interface TearFoundryExecutionBindingV3 { readonly format: "tear-foundry-execution-binding"; readonly schemaVersion: 3; readonly schedule: Readonly<{ id: string; revision: number; scheduleHash: string }>; readonly job: Readonly<{ id: string; jobHash: string; phase: TearFoundryJobV1["phase"] }>; readonly payload: TearFoundryExecutionBindingPayloadV1; readonly successorDeclaration: TearFoundryExecutionSuccessorDeclarationV3; readonly bindingHash: string; }
...
function declaration(value: unknown, phase: TearFoundryJobV1["phase"]): TearFoundryExecutionSuccessorDeclarationV3 {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("invalid Foundry V3 successor declaration"); const source = value as Record<string, unknown>;
  if ((phase === "created" || phase === "collecting") && source.kind === "exact-phase-payload") return Object.freeze({ kind: "exact-phase-payload", payload: payload(source.payload, successorPhase(phase)), nextDeclaration: declaration(source.nextDeclaration, successorPhase(phase)) });
  if (phase === "curating" && source.kind === "emitted-v2-training") { const next = declaration(source.nextDeclaration, "training"); if (next.kind !== "repeat-v2-training") throw new RangeError("Foundry V3 emitted declaration must lead to repeat training"); return Object.freeze({ kind: "emitted-v2-training", nextDeclaration: next }); }
  if (phase === "training" && source.kind === "repeat-v2-training") return Object.freeze({ kind: "repeat-v2-training" });
  throw new RangeError(`Foundry V3 successor declaration does not match ${phase}`);
}
function freezeV3(draft: Omit<TearFoundryExecutionBindingV3, "bindingHash">): TearFoundryExecutionBindingV3 { if (!text(draft.schedule.id) || !Number.isSafeInteger(draft.schedule.revision) || draft.schedule.revision < 1 || !hash(draft.schedule.scheduleHash) || !text(draft.job.id) || !hash(draft.job.jobHash)) throw new TypeError("invalid Foundry V3 execution binding"); const value = Object.freeze({ format: "tear-foundry-execution-binding" as const, schemaVersion: 3 as const, schedule: Object.freeze({ ...draft.schedule }), job: Object.freeze({ ...draft.job }), payload: payload(draft.payload, draft.job.phase), successorDeclaration: declaration(draft.successorDeclaration, draft.job.phase) }); return Object.freeze({ ...value, bindingHash: stableVerificationHash(value) }); }
export function createTearFoundryExecutionBindingV3(input: Omit<TearFoundryExecutionBindingV3, "format" | "schemaVersion" | "bindingHash">): TearFoundryExecutionBindingV3 { return freezeV3({ format: "tear-foundry-execution-binding", schemaVersion: 3, ...input }); }
export function parseTearFoundryExecutionBindingV3(value: unknown): TearFoundryExecutionBindingV3 { if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("invalid Foundry V3 execution binding"); const source = value as Record<string, unknown>; if (source.format !== "tear-foundry-execution-binding" || source.schemaVersion !== 3 || !hash(source.bindingHash)) throw new TypeError("invalid Foundry V3 execution binding"); const typed = source as unknown as TearFoundryExecutionBindingV3, { bindingHash, ...draft } = typed, parsed = freezeV3(draft); if (bindingHash !== parsed.bindingHash) throw new TypeError("Foundry V3 execution binding integrity mismatch"); return parsed; }
...
export class TearFoundryExecutionBindingVault {
  ...
  async bindV3(scheduleInput: TearFoundryJobScheduleV1, jobInput: TearFoundryJobV1, declarationInput: Readonly<{ payload: TearFoundryExecutionBindingPayloadV1; successorDeclaration: TearFoundryExecutionSuccessorDeclarationV3 }>): Promise<TearFoundryExecutionBindingV3> { const schedule = parseTearFoundryJobSchedule(scheduleInput), job = parseTearFoundryJob(jobInput); if (schedule.jobId !== job.id || schedule.jobHash !== job.jobHash || (await this.#jobs.get(job.id))?.jobHash !== job.jobHash) throw new RangeError("Foundry V3 execution binding current head changed"); const binding = createTearFoundryExecutionBindingV3({ schedule: { id: schedule.id, revision: schedule.revision, scheduleHash: schedule.scheduleHash }, job: { id: job.id, jobHash: job.jobHash, phase: job.phase }, ...declarationInput }); const key = `${V3_KEY}${binding.bindingHash}`, old = await this.#backend.get("analysis", key); if (old !== undefined) { try { const parsed = parseTearFoundryExecutionBindingV3(JSON.parse(old)); if (parsed.bindingHash === binding.bindingHash) return parsed; } catch { await this.#quarantine(key, old); } throw new RangeError("Foundry V3 execution binding key is corrupt"); } await this.#backend.commit(Object.freeze([{ store: "analysis", key, value: JSON.stringify(binding) }, { store: "analysis", key: `foundry-job-execution-binding-current:v3:${schedule.id}:${String(schedule.revision)}:${schedule.scheduleHash}`, value: binding.bindingHash }, { store: "indexes", key: `foundry-job-execution-binding:v3:${schedule.id}:${String(schedule.revision)}:${binding.bindingHash}`, value: JSON.stringify(Object.freeze({ scheduleHash: schedule.scheduleHash, jobHash: job.jobHash, phase: job.phase })) }])); return binding; }
  async getV3(bindingHash: string): Promise<TearFoundryExecutionBindingV3 | undefined> { if (!hash(bindingHash)) throw new TypeError("Foundry V3 execution binding hash is invalid"); const key = `${V3_KEY}${bindingHash}`, raw = await this.#backend.get("analysis", key); if (raw === undefined) return undefined; try { return parseTearFoundryExecutionBindingV3(JSON.parse(raw)); } catch { await this.#quarantine(key, raw); return undefined; } }
  async getCurrentV3(scheduleInput: TearFoundryJobScheduleV1): Promise<TearFoundryExecutionBindingV3 | undefined> { const schedule = parseTearFoundryJobSchedule(scheduleInput), key = `foundry-job-execution-binding-current:v3:${schedule.id}:${String(schedule.revision)}:${schedule.scheduleHash}`, value = await this.#backend.get("analysis", key); if (value === undefined) return undefined; const current = await this.getV3(value); return current?.schedule.scheduleHash === schedule.scheduleHash && current.schedule.revision === schedule.revision ? current : undefined; }
