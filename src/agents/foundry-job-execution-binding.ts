...
import { parseTearFoundryOfflineTrainingLaunch, type TearFoundryOfflineTrainingLaunchV2 } from "./foundry-job-offline-training";
...
type Offline = Readonly<{ manifest: Manifest; manifestHash: string; manifestRootHash: string; datasetHash: string; planHash: string; configurationHash: string; rewardHash: string }>;
...
function payload(value: unknown, phase: TearFoundryJobV1["phase"]): TearFoundryExecutionBindingPayloadV1 {
  ...
  if (phase === "curating" && source.kind === "offline-launch") {
    ...
    if (!frozen.every(hash)) throw new TypeError("invalid Foundry offline launch binding");
    return Object.freeze({ kind: "offline-launch", offline: Object.freeze({ manifest: manifest(offline.manifest), manifestHash: offline.manifestHash as string, manifestRootHash: offline.manifestRootHash as string, datasetHash: offline.datasetHash as string, planHash: offline.planHash as string, configurationHash: offline.configurationHash as string, rewardHash: offline.rewardHash as string }) });
  }
