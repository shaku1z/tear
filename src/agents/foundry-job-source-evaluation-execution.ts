...
import { parseTearOnlineRlSourceEvaluationPlan, TearOnlineRlSourceEvaluationExecutor } from "./online-rl-source-evaluation";
...
function parsePlanReceipt(value: unknown): TearFoundrySourceEvaluationPlanReceiptV1 {
...
export class TearFoundrySourceEvaluationExecutionExecutor {
  ...
  async execute(jobInput: TearFoundryJobV1, readinessInput: TearFoundryEvaluationReadinessReceiptV1, pairedInput: TearFoundryPairedEvaluationReadinessReceiptV1, planReceiptInput: TearFoundrySourceEvaluationPlanReceiptV1, executedAt: string): Promise<Readonly<{ job: TearFoundryJobV1; receipt: TearFoundrySourceEvaluationReceiptV1 }>> {
    ...
    try {
      const protocol = requireTearFoundryEvaluationProtocol(job), readiness = parseTearFoundryEvaluationReadinessReceipt(readinessInput), paired = parseTearFoundryPairedEvaluationReadinessReceipt(pairedInput); planReceipt = parsePlanReceipt(planReceiptInput);
      if (readiness.job.id !== job.id || paired.disposition !== "ready" || paired.job.resultJobHash !== job.jobHash || planReceipt.job.id !== job.id || planReceipt.job.jobHash !== job.jobHash || planReceipt.protocolHash !== protocol.protocolHash || planReceipt.pairedReadinessHash !== paired.receiptHash) throw new RangeError("Foundry source evaluation readiness changed");
      const rawPlan = await backend.get("analysis", `${PLAN_KEY}${planReceipt.planHash}`), rawLaunch = await backend.get("analysis", `foundry-job-online-training-launch:v1:${paired.launchHash}`);
      ...
      if (plan?.planHash !== planReceipt.planHash || plan.id !== protocol.id || plan.lineage.challengerCheckpointHash !== paired.checkpointHash || launch?.jobHash !== paired.job.sourceJobHash || launch.readinessReceiptHash !== readiness.receiptHashValue || launch.curriculum.planHash !== plan.lineage.curriculumPlanHash) throw new RangeError("Foundry source evaluation plan or online lineage changed");
      ...
      const result = await new TearOnlineRlSourceEvaluationExecutor(backend).execute(plan, launch.curriculum, offlineLaunch.plan, trajectory, online), next = transitionTearFoundryJob(job, "deciding", executedAt, "C34 paired source evaluation retained without a Foundry verdict"), output = receipt({ format: "tear-foundry-source-evaluation-receipt", schemaVersion: 1, job: { sourceJobHash: job.jobHash, resultJobHash: next.jobHash }, planReceiptHash: planReceipt.receiptHash, planHash: plan.planHash, resultHash: result.resultHash, disposition: "executed", executedAt });
      await this.#jobs.persistSuccessor(job, next, Object.freeze([{ store: "analysis", key: `${KEY}${output.receiptHash}`, value: JSON.stringify(output) }, { store: "indexes", key: `foundry-job-source-evaluation:${job.id}:${output.receiptHash}`, value: JSON.stringify(Object.freeze({ planHash: plan.planHash, resultHash: result.resultHash, disposition: output.disposition, promotional: false })) }])); return Object.freeze({ job: next, receipt: output });
    } catch (error) {
