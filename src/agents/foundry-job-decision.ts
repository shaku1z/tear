...
export interface TearFoundryDecisionContinuationV1 {
  readonly guards: (output: Readonly<{ job: TearFoundryJobV1; receipt: TearFoundryDecisionReceiptV1 }>) => readonly Readonly<{ store: "analysis"; key: string; expected?: string }>[],
  readonly writes: (output: Readonly<{ job: TearFoundryJobV1; receipt: TearFoundryDecisionReceiptV1 }>) => readonly GhostVaultWrite[],
}
...
export class TearFoundryDecisionExecutor {
  ...
  async decide(jobInput: TearFoundryJobV1, evaluationReceiptHash: string, decidedAt: string, continuation?: TearFoundryDecisionContinuationV1): Promise<Readonly<{ job: TearFoundryJobV1; receipt: TearFoundryDecisionReceiptV1 }>> {
    ...
    const complete = Object.freeze({ job: next, receipt: output });
    await this.#jobs.persistSuccessorIfMatches(job, next, Object.freeze([...(continuation?.guards(complete) ?? [])]), Object.freeze([{ store: "analysis", key: `foundry-job-decision:v1:${output.receiptHash}`, value: JSON.stringify(output) }, { store: "indexes", key: `foundry-job-decision:${job.id}:${output.receiptHash}`, value: JSON.stringify(Object.freeze({ disposition: output.disposition, resultHash: result.resultHash, promotional: false })) }, ...(continuation?.writes(complete) ?? [])]));
