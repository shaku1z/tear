...
export function createLiveScreenActionBindings(ports: ScreenActionBindingPorts) {
  return createScreenActionRouter({
    ...
    "game-agent.open": () => { ports.openGhostLab("botevidence"); },
    "run-monitor.open": () => { ports.openGhostLab("watch"); },
    "run-monitor.control": (action) => { ports.controlGhostLabWatch(action.command); },
