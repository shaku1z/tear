...
(function () {
  ...
  const interfaceComposition = createLiveInterfaceComposition({
    ...
    screens: {
      renderer: { canvas: ctx, ui: UI, width: W, height: H, screenRectangle: screenRect, time: () => uiT, enterAmount: () => eIn, enterSeconds: () => enterT, deltaSeconds: () => lastUiDt, mouse: () => ({ x: Input.mouseX, y: Input.mouseY }), scroll: () => listScroll, focus: () => focus, touch: () => Input.touchActive(), reducedMotion: () => A11Y.reducedMotion, enqueue: (button) => { uiButtons.push(button); } },
      ...
      runServices: { dependencies, reward: rewardRuntime, formatTime: fmtTime, clamp, trickColor, saveBest, awardCoins, cinema: CINEMA, clearFinale: () => { story.finale = null; }, terminateRun: (reason) => { RUN_LIFECYCLE.terminate(reason); }, addFloater, addShake, addFlash, requestPointer: requestLock },
