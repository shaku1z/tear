...
(function () {
  ...
  // full-screen rect INCLUDING the fullscreen overscan bleed Ã¢â‚¬â€ use for any fill that
  ...
  const interfaceComposition = createLiveInterfaceComposition({
    ...
    screens: {
      renderer: { canvas: ctx, ui: UI, width: W, height: H, time: () => uiT, enterAmount: () => eIn, scroll: () => listScroll, focus: () => focus, touch: () => Input.touchActive(), reducedMotion: () => A11Y.reducedMotion, enqueue: (button) => { uiButtons.push(button); } },
