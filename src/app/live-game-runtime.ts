...
import { createLiveAcademyScreen, createLiveFoundryScreen, createLiveGhostLabHome, createLiveInterfaceComposition, isRunDifficultySelection, isRunModeSelection } from "./live-interface-composition";
...
import { LivePlayerWatchController } from "./live-player-watch-controller";
...
(function () {
  ...
  const ghostV3Session = createLiveGhostRecordingSessionState(browserIndexedDb, { ...ghostV3Options, onFinalized: async (manifest, vault) => { await ghostV3Options.onFinalized?.(manifest, vault); await humanCalibration.capture?.finalized(manifest, vault); } });
  const ghostV3 = ghostV3Session.recorder();
  const academyScreen = createLiveAcademyScreen(browserIndexedDb, currentSignedInActor), foundryScreen = createLiveFoundryScreen(browserIndexedDb);
  ...
  void playerWatch.refresh(); const ghostLabHome = createLiveGhostLabHome(playerWatch);
  ...
  if (__TEAR_TEST_BUILD__ && TEST_MODE) void import("../tearbench/browser/live-runtime-bridge").then(({ installGhostV3BrowserInspector, installGhostVaultConditionalCommitInspector, installLiveTearRuntimeBridge }) => {
    ...
    installLiveTearRuntimeBridge({
      ...
      stopFrameLoop: () => { frameDriver.stop(); }, startFrameLoop: () => { frameDriver.start(({ deltaSeconds }) => { playerWatch.advance(); combatHost.frameCoordinator.run(deltaSeconds); }); }, pushAction: (action) => { Input.semantic.push(action); },
  ...
  frameDriver.start(({ deltaSeconds }) => { playerWatch.advance(); combatHost.frameCoordinator.run(deltaSeconds); });
})();
}
