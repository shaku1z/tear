...
export function createGhostLabRenderers(context: ScreenRenderContext) {
  return Object.freeze({
    ghostlab(view: GhostLabScreenView): void {
      ...
      ui.sectionLabel(canvas, "LOCAL WATCH", left, 596, cardWidth);
      ...
      ui.text(canvas, `STATUS / ${watch.status.toUpperCase()} / ${String(watch.decisions)} DECISIONS`, left + 20, 632, ui.t.type.caption, "left", ui.t.alpha.soft);
      ui.wrappedText(canvas, watch.detail, left + 20, 654, 390, 15, ui.t.type.micro, "left", ui.t.alpha.muted);
      ...
      context.enqueue({ x: left, y: 704, w: 200, h: 44, label, enabled: command !== "stop" || watch.status === "running" || watch.status === "paused", action: { type: "ghostlab.watch", command } });
      if (watch.status === "running" || watch.status === "paused") context.enqueue({ x: left + 214, y: 704, w: 200, h: 44, label: "STOP WATCH", action: { type: "ghostlab.watch", command: "stop" } });
      const right = 760;
