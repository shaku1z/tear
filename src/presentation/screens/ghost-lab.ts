...
export function createGhostLabRenderers(context: ScreenRenderContext) {
  return Object.freeze({
    ghostlab(view: GhostLabScreenView): void {
      ...
      ui.header(canvas, "REPLAY HUB", view.subtitle, context.enterAmount, ui.t.color.accent);
      const left = 260, top = 178, cardWidth = 430;
      ui.sectionLabel(canvas, "AVAILABLE LOCALLY", left, top - 28, cardWidth);
      // Embedded and availability-gated compatibility entries cannot become self-loop routes.
      view.routes.filter((route) => route.id !== "watch" && route.id !== "botevidence").forEach((route, index) => {
        ...
        const action = { type: REPLAY_HUB_ACTIONS.open, destination: route.id };
      ...
      if (watch.status === "ready" || watch.status === "running" || watch.status === "paused") {
        const command = watch.status === "ready" ? "start" : watch.status === "running" ? "pause" : "resume";
        const label = command === "start" ? "START RUN MONITOR" : command === "pause" ? "PAUSE RUN MONITOR" : "RESUME RUN MONITOR";
        context.enqueue({ x: left, y: 728, w: 200, h: 44, label, action: { type: RUN_MONITOR_ACTIONS.control, command } });
