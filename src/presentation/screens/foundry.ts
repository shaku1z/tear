  ...
  return Object.freeze({
    foundry(view: FoundryScreenView): void {
      ...
      if (view.status !== "ready") {
        ...
        if (profiles.length === 0) ui.text(canvas, "No local Foundry launch profile is stored.", panelX + 24, 312, ui.t.type.caption, "left", ui.t.alpha.muted);
        profiles.forEach((profile, index) => {
          ...
          context.enqueue({ x: panelX + 416, y: y - 22, w: 176, h: 26, label: eligible ? "START LOCAL CYCLE" : "BLOCKED", enabled: eligible,
            action: { type: "foundry.bootstrap", profileId: profile.profileId } });
        });
        ...
        if (jobs.length === 0) ui.text(canvas, "No validated local Foundry jobs are stored.", panelX + 24, jobsY + 64, ui.t.type.caption, "left", ui.t.alpha.muted);
        ...
        context.enqueue({ x: width / 2 - 110, y: jobsY + 20 + height, w: 220, h: 40, label: "REFRESH", action: { type: "foundry.refresh" } });
