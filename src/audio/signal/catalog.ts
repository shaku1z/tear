...
export function findWork(catalog: SignalCatalog, workId: string): MusicWork | null {
  return catalog.works.find((w) => w.id === workId) ?? null;
}

export function findStation(catalog: SignalCatalog, stationId: string): Station | null {
