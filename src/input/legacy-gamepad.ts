import type {
  GamepadSettings,
  LegacyGamepad,
  LegacyGamepadDependencies,
  PadPresetName,
  PadPresets,
} from "./legacy-input-contracts";

type GlyphStyle = Exclude<GamepadSettings["glyphStyle"], "auto">;

interface LegacyPulseActuator {
  pulse(value: number, duration: number): Promise<boolean>;
}

declare global {
  interface Gamepad {
    readonly hapticActuators?: readonly LegacyPulseActuator[];
  }
}

function isPadPresetName(name: string): name is PadPresetName {
  return name === "default" || name === "standard" || name === "tear" || name === "classic" || name === "split";
}

interface LegacyGamepadInternal extends LegacyGamepad {
  _activeT: number;
  _prev: Record<number, boolean | undefined>;
  _navT: number;
  _held: Set<string>;
  _resync: boolean;
  _tetherLatched: boolean;
  _tetherWas: boolean;
  _dtDir: number;
  _dtLastDir: number;
  _dtT: number;
  _stickWas: boolean;
  _release(): void;
  _detectDoubleTapDash(gamepad: Gamepad, horizontalAxis: number, dt: number): void;
  _glyphStyle(): GlyphStyle;
  _setHeld(code: string, on: boolean): void;
  _isDown(gamepad: Gamepad, index: number): boolean;
  _edge(gamepad: Gamepad, index: number): boolean;
  _down(gamepad: Gamepad, index: number): boolean;
  _edgeAny(gamepad: Gamepad, buttons: readonly number[]): boolean;
  _downAny(gamepad: Gamepad, buttons: readonly number[]): boolean;
}

// Temporary ESM Gamepad adapter for the classic game shell.
export function createLegacyGamepad(dependencies: LegacyGamepadDependencies): Readonly<{
  PAD: LegacyGamepad;
  PAD_PRESETS: PadPresets;
}> {
const { config: CONFIG, input: Input, window, navigator, semantic } = dependencies;
...
const PAD_CONFIG: GamepadSettings = CONFIG.pad = {
...
const PAD: LegacyGamepadInternal = {
  connected: false,
  index: -1,
  ...
  init() {
    window.addEventListener("gamepadconnected", (e) => {
      ...
      this.toastT = 3; this.toastText = "CONTROLLER CONNECTED — " + (e.gamepad.id || "gamepad").replace(/\(.*/, "").trim().slice(0, 28).toUpperCase();
    });
    window.addEventListener("gamepaddisconnected", (e) => {
      if (e.gamepad.index === this.index) {
        ...
        this._release(); Input.setPadTether(false); this._tetherLatched = false;   // never leave the tether stuck on after unplug
  ...
  setPreset(name) {
    this.preset = isPadPresetName(name) ? name : "default";
    this._release();
    Input.setPadTether(false); this._tetherLatched = false; this._dtDir = 0;
    this._resync = true;
    return this.preset;
  },
  ...
  _detectDoubleTapDash(gp, ax, dt) {
    ...
    this._dtT += dt;
    if (this._dtT > PAD_CONFIG.doubleTapWindow) this._dtDir = 0;
    if (dir !== 0 && this._dtLastDir === 0) {   // rising edge from neutral
      if (dir === this._dtDir && this._dtT <= PAD_CONFIG.doubleTapWindow) {
        Input.tDash = true;
        semantic.push({ type: "dash", x: dir * 1000, y: 0 });
        this._dtDir = 0;
      }
  ...
  rumble(strength, ms) {
    if (PAD_CONFIG.vibration === "off" || !this.connected) return;
    const scales: Readonly<Record<Exclude<GamepadSettings["vibration"], "off">, number>> = { low: 0.4, medium: 0.7, high: 1 };
    const scale = scales[PAD_CONFIG.vibration];
    const gp = navigator.getGamepads()[this.index];
    const modernActuator = gp?.vibrationActuator;
    const legacyActuator = gp?.hapticActuators?.[0];
    if (modernActuator === undefined && legacyActuator === undefined) return;
    const effectiveStrength = strength === undefined || strength === 0 ? 0.5 : strength;
    const duration = ms === undefined || ms === 0 ? 120 : ms;
    const mag = Math.max(0, Math.min(effectiveStrength * scale, 1));
    try {
      if (modernActuator !== undefined) void modernActuator.playEffect("dual-rumble", { duration, strongMagnitude: mag, weakMagnitude: mag * 0.7 });
      else void legacyActuator?.pulse(mag, duration);
    } catch { /* Controller haptics are optional. */ }
  },

  // resolved glyph for a Gamepad-API button index, honouring the glyph-style setting.
  _glyphStyle() {
    const s = PAD_CONFIG.glyphStyle;
    if (s !== "auto") return s;
    const id = (navigator.getGamepads()[this.index]?.id ?? "").toLowerCase();
  ...
  glyph(index) {
    ...
    const map: Readonly<Record<number, string>> = style === "playstation" ? PS : style === "generic" ? GN : XB;
  ...
  bindingLabel(action) {
    const P = PAD_PRESETS[this.preset];
    const arr = P[action]; if (arr.length === 0) return "";
    const shoulder = arr.find((i) => i >= 4 && i <= 7);
    const first = arr[0];
    return first === undefined ? "" : this.glyph(shoulder ?? first);
  },
  ...
  _isDown(gp, i) {
    ...
    if (i === 6 || i === 7) return b.pressed || b.value >= PAD_CONFIG.triggerThreshold;
    return b.pressed;
  },
  ...
  poll(dt, uiMode) {
    ...
    const actThresh = dead + PAD_CONFIG.activityBias;
    ...
    if (uiMode) {
      ...
      if (navKey && this._navT <= 0) {
        ...
        this._navT = PAD_CONFIG.menuRepeat;
    ...
    const deadL = PAD_CONFIG.deadL, deadR = PAD_CONFIG.deadR;
    ...
    const P = PAD_PRESETS[this.preset];
    if (this._edgeAny(gp, P.jump)) { Input.tJump = true; semantic.push({ type: "jump", phase: "pressed" }); }
    if (this._edgeAny(gp, P.dash)) {
      Input.tDash = true;
      semantic.push({ type: "dash", x: ((Input.right() ? 1 : 0) - (Input.left() ? 1 : 0)) * 1000, y: ((Input.down() ? 1 : 0) - (Input.up() ? 1 : 0)) * 1000 });
    }
    if (this._edgeAny(gp, P.throw)) {
      Input.rmb = true; this._tetherLatched = false;
      semantic.push({ type: "weapon", intent: "throw", phase: "pressed" });
    }   // throwing releases a toggled tether
    if (this._edge(gp, 9)) { Input.tPause = true; semantic.push({ type: "pause" }); }   // Start (universal pause)
    // optional directional double-tap dash (off by default) — never replaces the bound button
    if (PAD_CONFIG.doubleTapDash) this._detectDoubleTapDash(gp, ax, dt);
