...
const WEAPONS: readonly WeaponDefinition[] = [
  {
    ...
    damageProfile(ctx) {
      const W = ctx.config.weapons.sword;
  ...
  },
  {
    ...
    qualityMetric(ctx) { return clamp(len(ctx.blade.vx, ctx.blade.vy) / ctx.config.weapons.hammer.commitmentRef, 0, 1); },
    damageProfile(ctx) { return lerp(ctx.config.weapons.hammer.weakFloor, ctx.config.weapons.hammer.fullCommitMult, ctx.quality); },
    onHeldHit(ctx) { return { mechanic: "break", breakPower: ctx.damage * ctx.config.weapons.hammer.breakPerDamage * ctx.quality }; },
    onThrowLaunch(ctx) { ctx.blade._launchBallistic(ctx.config.weapons.hammer.meteorGravity); },
  ...
  },
  {
    ...
    damageProfile(ctx) {
      const W = ctx.config.weapons.spear;
      const hand = ctx.blade._lastHand;
      const reach = hand ? clamp(len(ctx.blade.tipX - hand.x, ctx.blade.tipY - hand.y) / (ctx.config.blade.maxReach + ctx.config.blade.length), 0, 1) : 0;
      return lerp(W.axialFloor, 1, ctx.quality) * (1 + reach * W.maxReachBonus);
    },
    onHeldHit(ctx) { return ctx.quality >= ctx.config.weapons.spear.driveThreshold ? { mechanic: "drive", force: ctx.config.weapons.spear.driveForce * ctx.quality } : null; },
    onThrowLaunch(ctx) { ctx.blade._launchStraight(); ctx.blade.linkT = ctx.config.weapons.spear.linkDuration * ctx.blade.channel("controlDuration"); },
  ...
  },
  {
    ...
    damageProfile(ctx) { return lerp(ctx.config.weapons.chainblade.tensionFloor, 1.12, ctx.quality); },
    onHeldHit(ctx) { return ctx.quality >= ctx.config.weapons.chainblade.fullTensionAt ? { mechanic: "drag", force: ctx.config.weapons.chainblade.dragForce * ctx.quality } : null; },
  ...
  },
  {
    ...
    damageProfile(ctx) { return 0.78 + ctx.blade.orbit * ctx.config.weapons.ringblade.orbitDamage; },
