...
export interface UpgradePlayerPort {
  maxHp: number;
...
export interface UpgradeBladePort {
  channelMods: {
    throwPower: number;
    throwSpeed: number;
    remoteRange: number;
  ...
  throwSizeMult: number;
  freeRecall: boolean;
}

export interface UpgradeEnemyPort {
  readonly x: number;
  readonly y: number;
...
export interface UpgradeEvent {
  ...
  enemy?: UpgradeEnemyPort;
  sourceEnemy?: UpgradeEnemyPort;
  enemies: readonly UpgradeEnemyPort[];
  cleanElimination: boolean;
  fx: {
    explode(x: number, y: number, color: string, scale: number): void;
    ring(x: number, y: number, radius: number, color: string): void;
  };
  dealAoE(x: number, y: number, radius: number, damage: number): void;
  dischargeStormbank?(mods: UpgradeMods): void;
  addOverrunStack?(mods: UpgradeMods): void;
  applySever?(enemy: UpgradeEnemyPort, tier: number): void;
}

export type UpgradeEventHandler = (event: UpgradeEvent) => void;

export interface UpgradeMods {
  weaponId?: string;
  onHit: UpgradeEventHandler[];
  onSwingHit: UpgradeEventHandler[];
  onKill: UpgradeEventHandler[];
  onParry: UpgradeEventHandler[];
  onPerfectParry: UpgradeEventHandler[];
  onSlam: UpgradeEventHandler[];
  onDashStart: UpgradeEventHandler[];
  onDashContact: UpgradeEventHandler[];
  onThrowLaunch: UpgradeEventHandler[];
  onThrowResolve: UpgradeEventHandler[];
  onThrowSecondary: UpgradeEventHandler[];
  onReturnHit: UpgradeEventHandler[];
  onWeaponCatch: UpgradeEventHandler[];
  onEnemyFirstDamaged: UpgradeEventHandler[];
  onEnemyDeath: UpgradeEventHandler[];
  onSkillKill: UpgradeEventHandler[];
  onReflectedHit: UpgradeEventHandler[];
  owned: Record<string, number>;
  ownedList: string[];
  throwRamp: number;
  deflectPierce: boolean;
  deflectSplit: boolean;
  airBonus: number;
  tempest: boolean;
  stormRecall: boolean;
  phantomDash: number;
  ...
  razorStun: boolean;
  stormBurst: boolean;
  phantomRefund: boolean;
  bleedHit: number;
  bleedDetonate: boolean;
  bleedNova: boolean;
  sunderHit: boolean;
  sunderShatter: boolean;
  sunderSpread: boolean;
  impale: number;
  impaleAll: boolean;
  impaleRecall: boolean;
  tempo: number;
  tempoMax: number;
  tempoSurge: boolean;
  backlash: number;
  backlashMark: boolean;
  backlashSurge: boolean;
  cinder: boolean;
  cinderSlow: boolean;
  cinderNova: boolean;
  concussive: number;
  ...
  stormbank: number;
  stormCharges: number;
  stormEchoes: { x: number; y: number; t: number; arcT: number }[];
  overrun: number;
...
export interface UpgradeRandomPort {
  next(): number;
}

export interface RollUpgradeOptions {
  random: UpgradeRandomPort;
  excludeIds?: readonly string[];
  forceSpecial?: boolean;
}

function newMods(): UpgradeMods {
  return {
...
const UPGRADES: readonly UpgradeDefinition[] = [
  ...
  { id: "rupture", name: "Rupture", unique: true, cat: "offense",
    ...
    apply: ({ mods }) => { mods.bleedHit = 1; mods.onHit.push((ev) => { if (mods.bleedHit && ev.enemy?.applyBleed) ev.enemy.applyBleed(mods.bleedHit); }); },
    tiers: [
  ...
  { id: "sunder", name: "Sunder", unique: true, cat: "offense",
    ...
    apply: ({ mods }) => { mods.sunderHit = true; mods.onHit.push((ev) => {
      const e = ev.enemy; if (!mods.sunderHit || !e?.applyMark) return;
...
function upWeight(u: UpgradeDefinition): number {
...
function rollUpgrades(n: number, mods: UpgradeMods | null | undefined, opts?: RollUpgradeOptions): UpgradeDefinition[] {
  ...
  const owned = mods?.owned ?? {};
  const excluded = new Set(opts.excludeIds ?? []);
  const pool = UPGRADES
    .filter((u) => !excluded.has(u.id))
    .filter((u) => !(u.unique && owned[u.id]))
    .filter((u) => !(u.maxStacks !== undefined && (owned[u.id] ?? 0) >= u.maxStacks))
    .map((u) => ({ u, w: upWeight(u) }));
  const out: UpgradeDefinition[] = [];
  while (out.length < n && pool.length) {
    let total = 0; for (const e of pool) total += e.w;
    let r = opts.random.next() * total, idx = 0;
    for (let i = 0; i < pool.length; i++) {
      const entry = pool[i];
      if (entry && (r -= entry.w) <= 0) { idx = i; break; }
    }
    const selected = pool.splice(idx, 1)[0];
    if (selected) out.push(selected.u);
  }
  ...
  if (opts.forceSpecial && !out.some((u) => u.tiers)) {
    ...
    if (specials.length) {
      ...
      const special = specials[Math.floor(opts.random.next() * specials.length)];
      if (ri >= 0 && special) out[ri] = special;
    }
  }
  return out;
}
...
function availableTierUps(mods: UpgradeMods): UpgradeDefinition[] {
  const out: UpgradeDefinition[] = [];
  for (const id in mods.owned) {
    const up = UPGRADES.find((u) => u.id === id);
    if (!up?.tiers) continue;
    const cur = mods.tier[id] ?? 1;       // current tier (1..3)
    if (cur - 1 < up.tiers.length) out.push(up);   // a further tier exists
  }
  return out;
}
function nextTierDesc(up: UpgradeDefinition, mods: UpgradeMods): string {
...
export { UPGRADES, applyUpgrade, availableTierUps, newMods, nextTierDesc, rollUpgrades, tierUp };
