...
export class EnvironmentState implements EnvironmentRuntimeState {
  ...
  constructor(stageId: string, worldId: string, configuration: Partial<EnvironmentRuntimeConfiguration> = {}, validators: readonly EnvironmentObjectValidationPort[] = []) {
    ...
    const ids = validators.map(({ id }) => id);
    assertUnique(ids, "validator");
  ...
  replace(snapshot: EnvironmentSnapshot): void {
    ...
    snapshot.combatObjects.forEach((object) => {
...
export function createEnvironmentState(stageId: string, worldId: string, configuration?: Partial<EnvironmentRuntimeConfiguration>, validators?: readonly EnvironmentObjectValidationPort[]): EnvironmentState {
  return new EnvironmentState(stageId, worldId, configuration, validators);
}
