...
- Each production world composition owns exactly one data-only environment runtime. Its bounded collections (fields, combat objects, and routes) use caller-owned world-scoped deterministic IDs. The authoritative fixed-step owner invokes one environment step that encloses gameplay in `pre-step -> active-fields -> collision-resolution -> post-commit`; production reset seams and detached replacement use the same lifecycle port, while presentation receives only a detached-safe view. The existing hazard codec now carries v2 environment collections with deterministic migration, identity/reference validation, canonical hash projection, structured observation, and invariant evidence; concrete field behavior remains later checkpoint work.
