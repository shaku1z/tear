...
- Each production world composition owns exactly one data-only environment runtime. Its bounded collections (fields, combat objects, and routes) use caller-owned world-scoped deterministic IDs. The authoritative fixed-step owner invokes one environment step that encloses gameplay in `pre-step -> active-fields -> collision-resolution -> post-commit`; production reset seams and detached replacement use the same lifecycle port, while presentation receives only a detached-safe view. The v2 hazard envelope, shared state owner, hash projection, observation codec, replay codec, and State Forge state codec understand only generic data-only extensions and reference-shaped keys. Biome modules own their concrete schemas and behavior: Bloom Wells, Root Links, Grafts, and Rootbound remain Verdant-owned; Aurora Tracks, Ghost Tracks, Pale routes, and White Hart remain Pale-owned. The architecture gate rejects direct or transitive cross-biome imports and biome-owned schema vocabulary in the neutral layer.
