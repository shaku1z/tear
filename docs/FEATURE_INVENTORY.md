...
- [ ] C35 human calibration source boundary — complete Ghost V3 capsules can be admitted only through separately consented pseudonymous, device-bound attestations, producing bounded aggregate features. Current capture does not issue an attestation, so no human record is inferred or admitted.
