...
- [ ] C36 durable Foundry job foundation — a content-addressed local job ledger fixes champion, held-corpus, evaluation, reward, invariant, budget, and stop-condition identities; it records only legal workflow transitions and safely resumes the current phase after restart. It does not yet schedule, train, evaluate, activate, promote, or present a policy.
