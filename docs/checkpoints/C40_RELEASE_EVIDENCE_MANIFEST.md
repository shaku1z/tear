...
[--subject <generated-artifact>] -- <explicit command>` runs an explicitly
...
written under `artifacts/tearbench/receipts/`; a failed command still receives
a failed receipt, but cannot satisfy the verifier.
