...
| Revision | 3.0 |
| Status | **Active implementation authority — VS3-C12-S3 green; VS3-C12-S4 next** |
...
- [x] **VS3-C12-S3** — Keep Rootbound directly damageable while Grafts exist. Permanent production-path evidence materializes all three canonical Grafts, advances them to active state, and proves Rootbound still rejects no ordinary hit, returns the full legal incoming amount from its limit seam, and loses HP through the normal damage path. Graft presence is absent from boss invulnerability/blocking logic; bounded mitigation is effect data to be resolved in S4, not a shield boolean.
