...
- **Setup craft, direct controller start, and Phase 4 behavior evidence (2026-07-25):** the typed setup renderer still authored the oracle's five-step risk pips, but `live-screen-renderers.ts` discarded that field at the final adapter, flattening every difficulty card. The adapter now carries the meter, the omitted weapon rating/weakness strip is restored, and the START hero advertises Square. Gamepad button 2 maps to the semantic primary-context channel, where only setup resolves it directly to START; the built controller matrix proves this without focus traversal. Phase 4's enemy matrix now runs at the live 120 Hz cadence, while Mirror tests use the real player integrator instead of a no-op actor double and prove a three-second pursuit produces live movement.
