...
DONE THIS STEP:      Slice 42 makes particle-system policy explicit at each constructed world's boundary.
PROVEN BY:           `pnpm check:c27a:slice42` passed: 5 focused files / 20 tests and the architecture gate. Two injected policies retain distinct budgets and reduced-motion behavior; the boundary rejects process-config and cosmetic-random imports in the particle module.
REMAINING HERE:      Extract a data-only generic bootstrap for the existing per-world simulation services; then continue presentation-policy isolation. App preference closures, Backdrop, renderer/UI, input, audio, persistence, and other adapters still prevent concurrent complete worlds and C27A remains blocking.
REMAINING TO C40:    C27 completion, C25 exit, and C28-C40 remain after C27A closes.
NEXT SLICE:          Create a data-only generic world bootstrap for configuration, clock, and named RNG, migrate live and detached construction to it, and leave presentation policy as an explicit outer adapter. Keep one runtime/scheduler and no normalized divergence.
