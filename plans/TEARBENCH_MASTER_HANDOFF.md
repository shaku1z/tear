...
- **C31:** active. A real C30 terminal candidate now receives a versioned,
  ...
  authority policy that rejects undeclared custody actors. C31 does not
...
NEXT SLICE:          Add quality, deduplication, corruption/outlier checks, and source metadata around held custody without admitting any non-held candidate into a corpus path; preserve the fail-closed admission boundary and do not write from the C30 callback.
