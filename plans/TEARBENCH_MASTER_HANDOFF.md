...
- **C27A:** eighteen shared-world architecture slices in this tree; blocking and incomplete.
- **C28-C40:** incomplete.
...
Eighteen C27A slices currently establish entity-construction separation, per-world entity-factory construction, generic per-world DOM-free context, its live adapter, combat collection ownership through `LiveGameHostState`, per-world transient opening/impact/frame-feel records, detached hydration/runtime foundations, focused tests, and physical browser diagnostics.
...
Both production combat phases now run detached and deterministically. Next: capture a live trace of the same seed and scripted actions through the TearBench bridge and compare it tick for tick against the detached trace — canonical actions, gameplay events, RNG snapshots, and state hashes. Treat each divergence as a recorded finding; the live host still owns wave orchestration, kill scoring, outcome, and cinematics, which the detached fixture stubs.
