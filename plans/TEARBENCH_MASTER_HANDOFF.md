...
NEXT SLICE:          Add immutable lineage-bound split assignment and durable pre-corpus manifests only for active curated held sources; keep hidden release-exam entries unreadable by ordinary trainer code and do not admit a source as a corpus sample without that separate governance boundary.
