...
Slice 53 completes the bounded browser AudioContext-handoff ownership step.
`createBrowserAudioContextHandoff()` keeps captured-context state private to
the app composition that constructs it; the required port flows through the
facade into the live compatibility adapter as a captured-context supplier.
Architecture rejects module-level captured state and a direct handoff import by
that adapter; direct two-handoff evidence proves one handoff's release leaves
the other untouched. `pnpm check:c27a:slice53` passed (10 files / 28 tests plus
architecture); the built browser audio contract and full built journeys passed;
fresh foundation passed 36 files / 130 tests and campaign victory passed 10
files / 36 tests and 1,176 browser transitions. The complete `pnpm check:c27a`
aggregate also passed. This preserves exactly one browser context for the app;
it does not establish concurrent audio graphs or audible/device parity. Next,
make the remaining browser input adapter composition-owned without changing
semantic input behavior or claiming full-world concurrency.
...
DONE THIS STEP:      Slice 53 makes the browser AudioContext handoff composition-owned, with local captured-context state and explicit facade/runtime supply.
PROVEN BY:           `pnpm check:c27a:slice53` passed: 10 focused files / 28 tests and architecture. Built browser audio and navigation/progression/playground/terminal/cinematic journeys passed. Fresh C27A foundation passed 36 files / 130 tests, 13 live scenarios and 40 detached comparisons; campaign victory passed 10 files / 36 tests and 1,176 transitions; the full `pnpm check:c27a` aggregate passed.
REMAINING HERE:      Make the remaining browser input adapter composition-owned, then isolate persistence adapter dependencies. Those adapters still prevent concurrent complete worlds and C27A remains blocking.
REMAINING TO C40:    C27 completion, C25 exit, and C28-C40 remain after C27A closes.
NEXT SLICE:          Make the remaining browser input adapter composition-owned without changing semantic input behavior. Do not claim physical-input/device parity or full live-world concurrency.
