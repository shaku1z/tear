...
REMAINING HERE:      C31 still needs detailed durable Academy inspection for individual recordings, corrections, consent/retention, and manifest revisions; trainer integration; population balance and style/skill interpretation; and account/cloud identity/deletion ownership. A reviewed sample and aggregate inspection are not a trainer corpus or trained policy. C30 target-host capacity remains external; C29's durable active-cinematic boundary remains separately open.
...
NEXT SLICE:          Extend the ordinary Academy route with durable individual recording/review/correction, consent/retention, and manifest inspection. Keep lesson-status claims separate unless their source is truly durable, and never treat the legacy in-memory corpus as the product surface.
