...
Slice 91 makes `MusicDirector` an explicit `LiveWorldComposition` member. The
live host now routes existing run orchestration, music observation, and frame
coordination through that per-world instance, and source architecture rejects
restoring host-local construction. The focused composition/music gate passed 5
files / 18 tests plus architecture; rebuilt campaign-victory browser proof
passed. This does not establish portable audio/device parity, a portable full
...
DONE THIS STEP:      Slice 91 makes music direction a per-world composition member rather than a live-host construction.
PROVEN BY:           Focused 5 files / 18 tests plus architecture and rebuilt campaign-victory browser proof passed. C27A remains blocking.
REMAINING HERE:      Extract real production-world construction and remaining outcome, combat, and app-bound audio/device state. Those boundaries still prevent concurrent complete worlds and C27A remains blocking.
