...
- **C32:** active. Local opaque policy artifacts now have a versioned,
  ...
  route consumes an active artifact receipt/action through semantic input.
  Ghost decision traces, evaluation, and broader Watch evidence remain open.
- **C33-C40:** incomplete.
...
NEXT SLICE:          Record bounded Ghost decision receipts from the active-artifact Class-A Watch path without widening observation or bypassing semantic input. Do not execute arbitrary payloads or connect a trainer. In C31, separately add complete pagination-safe record browsing and only authorized durable lesson status/actions.
