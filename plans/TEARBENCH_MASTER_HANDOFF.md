...
NEXT SLICE:          Seed a real browser Vault artifact and prove the Class-A Watch Agent emits its artifact receipt/action through semantic input, then record bounded decision evidence. Do not execute arbitrary payloads or connect a trainer. In C31, separately add complete pagination-safe record browsing and only authorized durable lesson status/actions.
