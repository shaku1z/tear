...
- **C27A:** twenty-four shared-world architecture slices in this tree; blocking and incomplete.
- **C28-C40:** incomplete.
...
Twenty-four C27A slices currently establish entity-construction separation, per-world entity-factory construction, generic per-world DOM-free context, its live adapter, combat collection ownership through `LiveGameHostState`, per-world transient opening/impact/frame-feel records, detached hydration/runtime foundations, focused tests, and physical browser diagnostics.
...
Live-versus-detached parity holds on a ten-scenario matrix: three ordinary runs, all five bosses, a 600-tick run, and a 903-tick terminal run, with every authoritative hash equal. Continue: comparable outward effects, then a win outcome, then campaign/gauntlet modes and a wave-boundary run. The boss-roster slice found that the Echo's per-tick combat read lived in the live adapter, leaving the boss inert anywhere else — the fourth product-level fix this matrix has produced.
