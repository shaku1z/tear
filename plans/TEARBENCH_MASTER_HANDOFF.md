...
Slice 61 completes the bounded frame-driver window step. The one existing
`RuntimeFrameDriver` receives `browserWindow`; source architecture rejects the
ambient constructor argument. `pnpm check:c27a:slice61` passed (4 files / 8
tests plus architecture), and rebuilt browser navigation, progression,
playground, terminal, and cinematic-preference journeys pass. This does not
introduce another scheduler, change frame/tick semantics, establish device
behavior, or create full-world concurrency. Next, supply the test-build
physical-input emitter's browser window without changing input semantics.
...
DONE THIS STEP:      Slice 61 supplies the one existing live frame driver with composition-owned browser-window capability.
PROVEN BY:           `pnpm check:c27a:slice61` passed: 4 focused files / 8 tests and architecture; rebuilt browser navigation, progression, playground, terminal, and cinematic-preference journeys passed. The previous fresh foundation and campaign-victory evidence remains recorded above; C27A remains blocking.
REMAINING HERE:      Supply the test-build physical-input emitter's browser-window capability, then isolate remaining persistence adapter dependencies. Those adapters still prevent concurrent complete worlds and C27A remains blocking.
REMAINING TO C40:    C27 completion, C25 exit, and C28-C40 remain after C27A closes.
NEXT SLICE:          Supply the test-build physical-input emitter's browser-window capability without changing input semantics. Do not claim persistence, durability/device parity, or full live-world concurrency.
