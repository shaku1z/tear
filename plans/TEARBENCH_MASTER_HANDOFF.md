...
NEXT SLICE:          Add the governed reviewed-sample boundary: materialize an explicitly curated, active source as a persisted sample tied to its exact capsule range while excluding revoked custody from every newly rebuilt corpus manifest and preserving hidden-exam isolation.
