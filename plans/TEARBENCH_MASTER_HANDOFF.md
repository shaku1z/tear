...
Slice 75 completes the bounded live combat-action generic-stat step. The shared
composition-owned adapter now supplies those add/max ports, and source
architecture rejects direct profile-stat access in the host.
`pnpm check:c27a:slice75` passed (6 files / 13 tests plus architecture), and
rebuilt campaign-victory browser proof passes. This does not establish durable
profile state, full-combat portability, migration, quota, crash recovery,
cross-device sync, or full-world concurrency.
...
DONE THIS STEP:      Slice 75 routes live combat-action generic add/max stat ports through the shared composition-owned adapter.
PROVEN BY:           `pnpm check:c27a:slice75` passed: 6 focused files / 13 tests and architecture; rebuilt campaign-victory browser proof passed. The previous fresh foundation and campaign-victory evidence remains recorded above; C27A remains blocking.
