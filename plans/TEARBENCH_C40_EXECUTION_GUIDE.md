...
- [x] `ghost/ghost-doctor` diagnoses a real corrupted capsule end to end — the C28 browser journey captures a V3 capsule, removes its persisted manifest index, tampers its actual IndexedDB chunk, then follows the normal Menu → Profile → Vault route. The rendered custody row says `NEEDS REPAIR`, while the durable maintenance receipt identifies the same capsule as unhealthy. Repair children preserve source bytes, record forensic quarantine copies, and commit the child plus its durable repair lineage together; player-initiated repair and the remaining exit evidence are still open.
