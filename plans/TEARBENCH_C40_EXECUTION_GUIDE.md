...
- [ ] C28 exit durability — records survive actual browser restart, schema migration, quota pressure, and interrupted writes. Browser evidence now preserves a legacy record through version-1 → version-2 migration and a second boot, and recovers a real interrupted journal/chunk into a terminal manifest with its journal removed and index rebuilt. Physical quota pressure remains independently required.
